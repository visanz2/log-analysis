...
let avg l =
	...
	( 
  	Big_int.div_big_int 
  	(
  		List.fold_left ( Big_int.add_big_int ) Big_int.zero_big_int l) 
  		(Big_int.big_int_of_int (List.length l)
  	)
	)
	else
	(
...
(" Arguments: MessageId [NodeID.LocalID] TaskId.\n This argument show the time use for arrive the message the one point a another point. The argument needs Message ID [NodeID.LocalID] and TaskId"
  ^ " The Message ID says de begin and the Task ID says the end point. For show all the message until ends, write TaskID = 0. \n"));



(* Latency Task Command Parameter *)
("--latencyTask", 
	Arg.Tuple 
    [
      Arg.Bool (* Option initial*) 
      (
        (
          fun key -> 
						 let varnode = try (key)
          		with Failure _ -> Printf.printf "\n That's not a boolean \n"; (true)
          		in optioninitial := varnode;
        )
      ); 

			Arg.Bool (* Option final*) 
      (
        (
          fun key -> 
						 let varnode = try (key)
          		with Failure _ -> Printf.printf "\n That's not a boolean \n"; (false)
          		in optionfinal := varnode;
							
        )
      ); 
			
			Arg.String (* Task ID initial*) 
      (
        (
          fun key -> 
						 let varnode = try int_of_string (key)
          		with Failure _ -> Printf.printf "\n That's not a number \n"; (-1)
          		in taskidsearchinitial := varnode;
        )
      ); 
      Arg.String (* Task Id last*)
      (
       fun key -> 
			 (
          let taskidsearchedend = try int_of_string (key)
          	with Failure _ -> Printf.printf "\n That's not a number \n"; (-1)
          	in
					let rec takemessage var =
						if (var < (List.length listlog)) then
        		(
							varmessagein := t_messageid_for_taskid (List.nth listlog var)  !taskidsearchinitial !optioninitial;
    					takemessage (var+1);  
            )
            else ();
     			in takemessage 0;

					let rec messagesloop varloop =	
						if(varloop < (List.length !varmessagein)) then
						(	
							let (nodeidinitial, localidinitial, timestampinitial) = List.nth !varmessagein varloop in
								let rec graphtask var =
  								if (var < (List.length listlog) && List.length !varlatency == 0) then
              		(
  									varlatency := !varlatency @ (t_latency_next (List.nth listlog var) nodeidinitial localidinitial);
          					graphtask (var+1);  
                  )
                  else ();
           			in graphtask 0;
									let rec searching ind =
										if ( ind > -1 && (List.length !varlatency > 0)) then
      							(
  											let rec loop  nodeidmessage localidmessage =
  												let rec logfileloop var =
    												if (var < (List.length listlog)) then
  													(
              									...
              									(
        												...
        												(
  																let rec loopson varson =
																		if ( varson < (List.length nextmessages)) then
																		(
																			let  (task_idin, timestampin, timestampout, nodeidson, localidson) = List.nth nextmessages varson in
          															if (task_idin == taskidsearchedend) then
          															(
																					if (!optionfinal == true) then
																					(
																						avglatency := !avglatency @ [ ( Big_int.sub_big_int timestampin timestampinitial)];
																					)
																					else
																					(
																						avglatency := !avglatency @ [ ( Big_int.sub_big_int timestampout timestampinitial)];
																					);
          															)
          															else
          															(
          																loop nodeidson localidson;
																					loopson (varson +1);
          															);
    																)	
  																in loopson 0; 
													...
													in logfileloop 0; 
  											in loop nodeidinitial localidinitial; varlatency := List.tl !varlatency;  searching (ind+1);
										)
										else
										(
										)
      						...
      						messagesloop (varloop+1);
						)else()
					in messagesloop 0; Printf.printf "Latency: %s\n" (Big_int.string_of_big_int (avg !avglatency));messageoptions:= true;
			 )
			)
		]
, 

(" Arguments: optionInitial optionFinal TaskIdInitial TaskIdFinal.\n
This argument show the time use for arrive the message the one point a another point. 
In this argument is possible to define if we want the first task inclusive or exclusive and the last task inclusive or exclusive.
For this we use the arguments optionInitial and optionFinal. If you write true that's mean inclusive, if it is false thant's mean exclusive.
The TaskIdInitial says de begin and the TaskIdFinakl says the end point. If the latency is 0, there isn't any connection between this tasks. \n"));
